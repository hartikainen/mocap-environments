...
def process_motion(
    ...
    if tracking_result["mujoco_states"]["qpos"].shape[0] != keyframes.shape[0]:
        logging.info(f"`track_motion` terminated early for keyframe_path='{str(keyframe_path)}'")
    ...
    try:
        ...
        logging.error(f"`playback_motion` failed for keyframe_path='{str(keyframe_path)}'")
        raise
