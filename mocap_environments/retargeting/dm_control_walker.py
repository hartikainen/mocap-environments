...
def compute_inverse_kinematics_qpos_qvel(
    ...
    with physics.reset_context():
        root_joint_indices = physics.named.data.qpos.axes.row.convert_key_item(
            list(root_joints_names)
        )
        physics.named.data.qpos[root_joint_indices] = ik_result_0.qpos[
            root_joint_indices
        ]
