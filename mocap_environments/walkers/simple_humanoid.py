...
class _CMUHumanoidBase(legacy_base.Walker, metaclass=abc.ABCMeta):
    ...
    def _build(
        ...
        if include_face:
            ...
            face = head.add("body", name="face", pos=(0.0, 0.039, face_forwardness))
            face.add(
                "geom",
                type="capsule",
                name="nose",
    ...
    def end_effectors(self):
        return (
            self._mjcf_root.find("body", "rradius"),
    ...
    def marker_geoms(self):
        return (
            self._mjcf_root.find("geom", "rradius"),
...
class CMUHumanoidObservables(legacy_base.WalkerObservables):
    ...
    def egocentric_camera(self):
        ...
        return observable.MJCFCamera(
            self._entity.egocentric_camera, width=64, height=64, scene_option=options
        )
